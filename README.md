...
* **Интеграционные тесты с использованием TestContainer** Написал тесты как для DAO-слоя, так и для Контроллеров (использовал этот ресурс как пример https://testcontainers.com/guides/testing-spring-boot-rest-api-using-testcontainers/)
...
## Использование ИИ
Я использовал Gemini для наполнения БД тестовыми данными, а также задавал вопросы касательно 
Spring Security (swagger не работал), и в постман передавал переменную {{end/startTime}} 
в тело запроса и была проблема в отсутствием кавычек
В корне проекта лежит .pdf документ (task_tracker_full_dialogue.pdf) с промтами (я попросил чат наш диалог конвертировать в pdf)
Применение Bearer Authentication (JWT) подсмотрел у https://struchkov.dev/blog/ru/jwt-implementation-in-spring/
